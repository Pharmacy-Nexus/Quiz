...
function renderStudyQuestion() {
  ...
  if (statusWrap) {
    statusWrap.innerHTML = `<div class="rounded-[1.5rem] border border-outline-variant/15 bg-gradient-to-br from-surface-container-lowest to-surface-container-low p-4 shadow-[0_12px_28px_rgba(0,21,27,0.05)]"><div class="flex items-center justify-between gap-3 mb-4"><div><h3 class="text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-1">Question Status</h3><p class="text-sm text-on-surface-variant">Keep track without distracting the reading area.</p></div><label class="flex items-center gap-2 text-[11px] font-bold text-on-surface-variant uppercase tracking-widest cursor-pointer whitespace-nowrap"><input id="study-auto-next-toggle" type="checkbox" class="text-primary focus:ring-primary" ${appState.studyUi?.autoNext ? 'checked' : ''}/> Auto-next</label></div><div class="grid grid-cols-3 gap-2 mb-4"><div class="rounded-xl bg-primary-fixed/25 p-3 text-center"><p class="text-[10px] font-bold uppercase tracking-widest text-primary mb-1">Answered</p><p class="text-lg font-black text-primary">${answeredTotal}</p></div><div class="rounded-xl bg-surface-container-high p-3 text-center"><p class="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-1">Unanswered</p><p class="text-lg font-black text-on-surface">${unansweredTotal}</p></div><div class="rounded-xl bg-tertiary/10 p-3 text-center"><p class="text-[10px] font-bold uppercase tracking-widest text-tertiary mb-1">Current</p><p class="text-lg font-black text-tertiary">${humanIndex}</p></div></div><div class="flex flex-wrap gap-2 mb-4">${answerMap}</div><div class="flex flex-wrap gap-3 text-xs font-semibold text-on-surface-variant"><div class="flex items-center gap-2"><span class="w-2.5 h-2.5 rounded-full bg-primary-fixed inline-block"></span> Answered</div><div class="flex items-center gap-2"><span class="w-2.5 h-2.5 rounded-full bg-surface-container-high inline-block"></span> Unanswered</div><div class="flex items-center gap-2"><span class="w-2.5 h-2.5 rounded-full bg-primary inline-block"></span> Current</div></div></div>`;
  }
