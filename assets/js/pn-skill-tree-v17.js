(function () {
  ...
  function cleanName(name) {
    ...
    if (['biochemictry','biochemstry','biochemitry','biochemistryroadmap','biochemictryroadmap'].includes(compact)) return 'Biochemistry';
  ...
  function openHomeTopic(subjectId, topicId, event) {
    if (event) {
      event.preventDefault();
      event.stopPropagation();
    }
    ...
    if (!sid) return;
    try { localStorage.setItem('pn_last_subject_id_v14', sid); } catch (_) {}

    // Home skill-tree nodes should open the actual topic route when possible.
    // If the original app has not exposed selectTopic yet, fall back to opening the subject.
    if (tid && typeof window.selectTopic === 'function') {
      try {
        window.selectTopic(sid, tid);
        return;
      } catch (err) {
        console.warn('Home skill node could not open topic, falling back to subject:', err);
      }
    }
    openSubject(sid, event);
  }

  window.pnOpenHomeTopic = openHomeTopic;
  ...
  function homeNodeLayout(items) {
    ...
    if (count <= 3) {
      const xs = count === 1 ? [36] : count === 2 ? [28, 50] : [22, 42, 62];
      return items.map((item, i) => ({ ...item, x: xs[i], y: count === 3 && i === 1 ? 60 : 48 }));
    }
    const slots = [
      [25, 34], [50, 48], [75, 34],
  ...
  async function renderDynamicHomeSkillTree() {
    ...
    const compactHome = shown.length <= 3;
    const root = compactHome ? [42, 18] : [50, 14];
    ...
    const nodes = shown.length ? shown.map(item => `
          <button type="button" class="pn-tree-node pn-home-topic-node ${item.status} ${recommended && item.topicId === recommended.topicId ? 'recommended' : ''}" style="left:${item.x}%;top:${item.y}%" onclick="pnOpenHomeTopic('${esc(subject.id)}', '${esc(item.topicId)}', event)" aria-label="Open ${esc(item.name || cleanName(subject.name))}">
    ...
    section.innerHTML = `
      ...
      <div class="pn-skill-map" aria-label="${esc(cleanName(subject.name))} skill tree preview">
        <div class="pn-skill-map-inner has-panel ${shown.length <= 3 ? 'compact-roadmap' : ''}">
          ...
          <button type="button" class="pn-tree-node root mastered" style="left:${compactHome ? 42 : 50}%;top:${compactHome ? 18 : 14}%" onclick="pnOpenSubject('${esc(subject.id)}', event)"><strong>${esc(cleanName(subject.name))}</strong><small>Last opened subject</small></button>
          ${nodes}
          <aside class="pn-home-node-panel">
            ...
            <button type="button" onclick="pnOpenHomeTopic('${esc(subject.id)}', '${esc(selected.topicId || '')}', event)">Open route <span class="material-symbols-outlined">arrow_forward</span></button>
          </aside>
        </div>
      </div>`;
  ...
  function layoutTopicNodes(items) {
    ...
    // V17: keep every node inside a safe visual zone so outlines, hover states,
    // and long titles do not get clipped by the dark board.
    if (mobile) {
      const step = Math.min(13, 70 / Math.max(count - 1, 1));
      return items.map((item, i) => ({ ...item, x: 50, y: Math.min(86, 18 + i * step) }));
    }
    ...
    if (tablet) {
      return items.map((item, i) => {
        ...
        return { ...item, x: isLeft ? 32 : 68, y: Math.min(84, 24 + row * 18) };
      });
    }

    if (count === 1) return items.map(item => ({ ...item, x: 50, y: 58 }));
    if (count === 2) return items.map((item, i) => ({ ...item, x: i === 0 ? 34 : 66, y: 58 }));
    if (count <= 4) {
      const slots = [[30, 54], [70, 54], [30, 76], [70, 76]];
      return items.map((item, i) => ({ ...item, x: slots[i][0], y: slots[i][1] }));
    }
  ...
  function renderTopicSkillTree() {
    ...
    const subjectName = cleanName(document.querySelector('#page-topics h1')?.textContent?.trim() || 'Subject');
    ...
    existing.className = `pn-skill-shell pn-topic-tree ${items.length <= 4 ? 'compact' : ''}`;
    existing.style.setProperty('--pn-topic-count', String(items.length));
    existing.style.setProperty('--pn-topic-rows', String(Math.max(1, Math.ceil(items.length / (window.matchMedia('(max-width: 980px)').matches ? 2 : 3)))));
    existing.innerHTML = `
      <div class="pn-topic-tree-head">
        <div><span class="kicker"><span class="material-symbols-outlined">account_tree</span> Skill Tree</span><h2>${esc(subjectName)} Learning Path</h2><p>Follow the subject visually. Completed nodes glow green, weak topics are marked clearly, and the recommended node is highlighted for your next move.</p></div>
