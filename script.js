...
async function initializeData() {
  ...
  const [seedSubjects, seedTopics, seedQuestions, seedQuizsets] = await Promise.all([
  ...
  if (!localStorage.getItem(STORAGE_KEYS.settings)) save(STORAGE_KEYS.settings, { owner: '', repo: '', branch: 'main', token: '' });
  if (!localStorage.getItem(STORAGE_KEYS.progress)) save(STORAGE_KEYS.progress, { attempts: [], recent: [] });
  ...
  appData = {
    ...
    settings: load(STORAGE_KEYS.settings, { owner: '', repo: '', branch: 'main', token: '' }),
    progress: load(STORAGE_KEYS.progress, { attempts: [], recent: [] })
  };
...
function renderAdmin() {
  ...
  const renderQuestionsAdmin = () => {
    ...
    fillSelect(topicSelect, appData.topics.filter((t) => t.subjectId === subjectId));
